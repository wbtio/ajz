...
export default function LinksPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-white pt-28 sm:pt-32 lg:pt-36 pb-16 sm:pb-20 lg:pb-28" dir={dir}>
      ...
      <Container className="relative z-10">
        <motion.section
          ...
          className="relative overflow-hidden rounded-xl border border-[#0b1426]/10 bg-white p-5 shadow-sm sm:p-7 lg:p-10"
        >
          <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#0b1426]/10 to-transparent opacity-90" />
          <div className="pointer-events-none absolute -top-28 right-0 h-56 w-56 rounded-full bg-slate-400/5 blur-3xl" />
          ...
          <div className="relative z-10">
            ...
            >
              <span className="mb-4 inline-flex min-h-9 items-center rounded-sm border border-[#0b1426]/10 bg-[#f5f7fa] px-3.5 py-1 text-xs font-semibold uppercase tracking-wider text-[#0b1426]/80">
                {locale === 'ar' ? 'بوابات وخدمات رسمية' : 'Official Portals & Services'}
              </span>
              <h1 className="mb-4 text-3xl font-bold tracking-tight text-[#0b1426] sm:text-4xl lg:text-5xl">
                {locale === 'ar' ? 'الروابط المهمة' : 'Important Links'}
              </h1>
            ...
            <motion.div
              ...
              className="relative mb-6 overflow-hidden rounded-xl border border-[#0b1426]/10 bg-white px-3 py-2.5 shadow-sm sm:px-4 sm:py-2.5"
            >
              <div className="absolute inset-0 bg-gradient-to-r from-white via-white to-slate-500/[0.04]" />
              <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#0b1426]/10 to-transparent opacity-80" />
              <div className="relative z-10">
                <div
                  dir="ltr"
                ...
                >
                  <div
                    key={locale}
                    className={[
                      'flex w-max items-center gap-2.5',
                  ...
                  >
                    {SCROLL_ITEMS.map((country, i) => (
                      <div
                        ...
                        className="group/item flex h-12 w-12 shrink-0 items-center justify-center rounded-sm border border-[#0b1426]/10 bg-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-[#8B0000]/30 hover:shadow-md sm:h-14 sm:w-14"
                      >
                        <Icon icon={`circle-flags:${country.code}`} className="h-7 w-7 sm:h-9 sm:w-9" />
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {categories.map((category, index) => {
                ...
                return (
                  <motion.details
                    ...
                    className="group overflow-hidden rounded-xl border border-[#0b1426]/10 bg-white transition-all duration-300 hover:border-[#0b1426]/20"
                  >
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-3.5 sm:p-4 [&::-webkit-details-marker]:hidden select-none outline-none">
                      <div className={`flex items-center gap-3.5 ${isRTL ? 'flex-row-reverse text-right' : ''}`}>
                        <div
                          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-sm transition-all duration-300 group-open:bg-[#0b1426]"
                          style={{ backgroundColor: `${accent}12` }}
                        >
                          <CategoryIcon className="h-4.5 w-4.5 transition-colors duration-300 group-open:text-white" style={{ color: accent }} />
                        </div>
                        <div>
                          <h2 className="text-base sm:text-lg font-bold text-[#0b1426]">
                      ...
                      <div className={`flex items-center gap-2.5 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <span className="rounded-sm border border-[#0b1426]/10 bg-[#f5f7fa] px-2.5 py-0.5 text-xs font-semibold text-[#0b1426]/80">
                          {categoryLinks.length} {locale === 'ar' ? 'روابط' : 'links'}
                        </span>
                        <ChevronDown className="h-4.5 w-4.5 text-slate-400 transition-transform duration-300 group-open:rotate-180" />
                      </div>
                    </summary>

                    <div className="border-t border-[#0b1426]/5 bg-[#f5f7fa]/60 p-3 sm:p-4">
                      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                        {categoryLinks.map((link) => (
                          <a
                            ...
                            className="group/link relative overflow-hidden rounded-sm border border-[#0b1426]/10 bg-white p-3 sm:p-3.5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-[#8B0000]/30 hover:shadow-md"
                          >
                            <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#8B0000]/20 to-transparent opacity-0 transition-opacity duration-300 group-hover/link:opacity-100" />
                            <div className={`relative z-10 flex items-center justify-between gap-3 ${isRTL ? 'flex-row-reverse text-right' : ''}`}>
                              <div className="flex-1 min-w-0">
                                <h3 className="text-sm font-semibold text-slate-900 transition-colors duration-200 group-hover/link:text-[#8B0000] truncate">
                                  {locale === 'ar' ? link.title_ar : link.title_en}
                                </h3>
                                {link.country && (
                                  <div className={`flex items-center gap-1 mt-0.5 text-xs text-slate-400 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
                                    <MapPin className="h-3 w-3 shrink-0" />
                                    <span className="truncate">{locale === 'ar' ? link.country.name_ar : link.country.name_en}</span>
                                  </div>
                                )}
                              </div>
                              <ExternalLink className="h-3.5 w-3.5 shrink-0 text-slate-400 transition-all duration-200 group-hover/link:text-[#8B0000] group-hover/link:translate-x-0.5 rtl:group-hover/link:-translate-x-0.5" />
                            </div>

                            {(locale === 'ar' ? link.description_ar : link.description_en) && (
                              <p className={`mt-2 text-xs leading-relaxed text-slate-500 line-clamp-2 ${isRTL ? 'text-right' : ''}`}>
                                {locale === 'ar' ? link.description_ar : link.description_en}
                              </p>
                            )}
                          </a>
            ...
            {links.length === 0 && (
              <div className="mt-6 rounded-xl border border-[#0b1426]/10 bg-white py-16 text-center shadow-sm">
                <div className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-sm bg-[#8B0000]/6 text-[#8B0000]">
                  <Globe className="h-8 w-8" />
                </div>
                <h3 className="mb-2 text-xl font-bold text-[#0b1426]">
