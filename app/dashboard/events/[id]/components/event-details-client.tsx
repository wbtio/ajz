...
export function EventDetailsClient({
    ...
    const steps = [
        { id: 1, name: 'تسجيل الفعالية', desc: 'نشر البيانات الأساسية للموقع', icon: Calendar, completed: step1Completed },
        { id: 2, name: 'فريق التصميم', desc: 'إعداد وتجهيز وتدقيق الصور والفيديو', icon: Palette, completed: step2Completed },
        { id: 3, name: 'النشر والترويج', desc: 'توزيع المنشورات عبر السوشيال ميديا', icon: Share2, completed: step3Completed },
    ]
            ...
            <div className="bg-white border border-slate-200/70 rounded-xl px-4 py-3 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        <h1 className="text-xl font-bold text-slate-900 truncate">{event.title_ar || event.title}</h1>
                        <div className="flex items-center gap-2 mt-1 text-xs text-slate-500">
                            <span className={`inline-flex px-2 py-0.5 font-semibold rounded-full ${
                                ...
                                {event.status === 'published' ? 'نشط' :
                                 event.status === 'completed' ? 'مكتمل' :
                                 event.status === 'cancelled' ? 'ملغي' : 'مسودة'}
                            </span>
                            <span className="text-slate-300">•</span>
                            <span className="flex items-center gap-1 whitespace-nowrap"><Calendar className="w-3.5 h-3.5 text-slate-400" /> {new Date(event.date).toLocaleDateString('ar-IQ')}</span>
                            <span className="text-slate-300 hidden sm:inline">•</span>
                            <span className="flex items-center gap-1 truncate"><MapPin className="w-3.5 h-3.5 text-slate-400 shrink-0" /> <span className="truncate">{event.location_ar || event.location}</span></span>
                        </div>
                    </div>
                </div>
                <Link href={`/events/${event.id}`} target="_blank" className="shrink-0">
                    <Button variant="outline" size="sm" className="border-slate-200 text-slate-700 h-9 rounded-xl hover:bg-slate-50 text-xs gap-1">
                        ...
                        <span className="hidden sm:inline">معاينة بالموقع</span>
                    </Button>
                </Link>
            </div>
                            ...
                            return (
                                <li key={s.id} className="flex-1 min-w-[72px] sm:min-w-0">
                                    <button
                                        ...
                                        aria-label={`الخطوة ${s.id}: ${s.name}${s.completed ? '، مكتملة' : ''}${!hasAccess ? '، مغلقة' : ''}`}
                                        className={cn(
                    ...
                    <Card className="border-rose-100 bg-rose-50/20 shadow-md py-16 text-center max-w-2xl mx-auto rounded-2xl">
                        <CardContent className="space-y-4">
                            ...
                            <p className="text-slate-500 text-sm max-w-md mx-auto leading-relaxed">
                                نعتذر، لا تملك الصلاحيات الإدارية المطلوبة لمشاهدة أو تعديل الخطوة {activeStep} ({steps.find(s => s.id === activeStep)?.name}).
                            </p>
