export const translations = {
  ar: {
    // Navigation
    nav: {
      home: "الرئيسية",
      services: "الخدمات",
      about: "من نحن",
      events: "الفعاليات",
      sectors: "الأقسام",
      blog: "المدونة",
      training: "التدريب",
      contact: "تواصل معنا",
      partners: "الشراكات",
      links: "الروابط",
      calendar: "التقويم",
      invitationSupport: "دعم الدعوات",
    },
    // Partners Page
    partnersPage: {
      hero: {
        title: "شركاؤنا",
        subtitle:
          "معاً نحو مستقبل مستدام! انضم لشبكة شركائنا وساهم في صنع التغيير.",
      },
      corporate: {
        title: "الشراكات المؤسسية: تأثير مستدام لعلامتك التجارية",
        description:
          "اصنع فرقاً في العالم من خلال التعاون معنا لتعزيز المسؤولية البيئية والاجتماعية، وجذب العملاء الواعين، والمساهمة في تحقيق أهداف التنمية المستدامة.",
        types: {
          commercial: {
            title: "الشراكات التجارية",
            description:
              "دعم المنتجات والخدمات الصديقة للبيئة أو تمويل المشاريع المستدامة.",
          },
          marketing: {
            title: "الشراكات التسويقية",
            description:
              "الترويج المشترك للمبادرات المستدامة عبر منصات التواصل الاجتماعي.",
          },
          technical: {
            title: "الدعم التقني",
            description:
              "توفير حلول تقنية مبتكرة، مثل تحسين كفاءة الطاقة وتحليل البيانات البيئية.",
          },
          social: {
            title: "المسؤولية الاجتماعية",
            description: "رعاية حملات التوعية وبرامج التدريب المجتمعي.",
          },
        },
        contact: "تواصل معنا عبر البريد الإلكتروني أو املأ استمارة التعاون.",
        cta: "انضم لشبكة شركائنا اليوم!",
      },
      volunteers: {
        title: "طاقات تطوعية: كن جزءاً من الحل!",
        description:
          "ساهم في التغيير البيئي والاجتماعي، اكتسب خبرة قيمة، وكن جزءاً من مجتمع يهتم بالاستدامة.",
        roles: {
          graphic: {
            title: "التصميم الجرافيكي",
            description: "إنشاء محتوى بصري لدعم حملات التوعية.",
          },
          management: {
            title: "إدارة المشاريع",
            description: "تنظيم الفعاليات ومتابعة المبادرات المستدامة.",
          },
          marketing: {
            title: "التسويق الرقمي",
            description: "إدارة منصات التواصل الاجتماعي وكتابة المحتوى.",
          },
          community: {
            title: "الدعم المجتمعي",
            description: "تدريب الأفراد والمشاركة في الأنشطة الميدانية.",
          },
          dev: {
            title: "تطوير الويب",
            description: "تحسين الموقع وتطوير أدوات رقمية خضراء.",
          },
        },
        cta: "سجل كمتطوع الآن!",
      },
      dialog: {
        partner: {
          title: "ابدأ الشراكة معنا",
          description:
            "اختر الطريقة التي تفضلها للتواصل معنا وبدء رحلة الشراكة.",
        },
        volunteer: {
          title: "انضم لفريق المتطوعين",
          description: "اختر الطريقة التي تناسبك للتسجيل والانضمام إلينا.",
        },
        ideas: {
          title: "شارك فكرتك معنا",
          description: "اختر الطريقة التي تفضلها لمشاركة فكرتك وسنتواصل معك.",
        },
        email: "إرسال بريد إلكتروني",
        form: "ملء الاستمارة",
      },
      ideas: {
        title: "لديك فكرة لمشروع مستدام؟ نحن هنا لدعمك!",
        description:
          "نساعد الأفراد والمؤسسات على تحويل الأفكار إلى مشاريع حقيقية تخدم البيئة والمجتمع.",
        cta: "شارك فكرتك معنا الآن، وسنناقش كيفية دعمها وتطويرها!",
      },
    },
    // Calendar Page
    calendarPage: {
      title: "تقويم الفعاليات",
      subtitle: "استكشف جدول الفعاليات والمعارض القادمة طوال العام",
      search: "ابحث عن فعالية...",
      filterCategory: "التصنيف",
      filterLocation: "الموقع",
      viewPast: "عرض الفعاليات السابقة",
      allCategories: "جميع التصنيفات",
      allLocations: "جميع المواقع",
    },
    // Hero Section
    hero: {
      badge: "المنصة العراقية للمعارض والمؤتمرات الدولية",
      title: "JAZ",
      subtitle: "Joint Annual Zone to Your Place",
      description:
        "نربطك بالمعارض والمؤتمرات الدولية عبر حضور محلي قوي وشبكة علاقات مهنية تمتد إلى أهم الأسواق.",
      browseEvents: "تصفح المعارض القادمة",
      contactUs: "تواصل معنا",
      stats: {
        exhibitions: "معرض ناجح",
        visitors: "زائر",
        experience: "سنوات خبرة",
      },
    },
    homepage: {
      hero: {
        title: "بوابة العراق للمعارض والشراكات الدولية",
        description: "نربط المؤسسات بالمعارض الدولية ونبني شراكات مستدامة.",
        ctaCooperation: "طلب تعاون",
        ctaParticipation: "المشاركة الدولية",
        ctaPartner: "شراكة في العراق",
        ctaInvitation: "دعم الدعوات",
        statPartnerCountriesVal: "15",
        statPartnerCountriesLabel: "دولة شريكة",
        statActivePartnersVal: "120",
        statActivePartnersLabel: "شريك نشط",
        statJointInitiativesVal: "250",
        statJointInitiativesLabel: "مبادرة مشتركة",
        statYouthBeneficiariesVal: "1000",
        statYouthBeneficiariesLabel: "شاب مستفيد",
      },
      sectors: {
        title: "قطاعات تركيزنا",
        subtitle: "أربعة محاور نركّز عليها لربط المؤسسات العراقية بالأسواق العالمية.",
        viewAll: "استعرض كل الأقسام",
        medicalTitle: "الرعاية الصحية وعلوم الحياة",
        medicalDesc: "ربط المؤسسات الصحية بالابتكار العالمي والتطورات الطبية.",
        techTitle: "التحول الرقمي والتكنولوجيا",
        techDesc: "تمكين المستقبل الرقمي للعراق من خلال معارض التكنولوجيا والشراكات العالمية.",
        indTitle: "التطوير الصناعي والتجاري",
        indDesc: "تعزيز النمو الصناعي، فرص التجارة، والتعاون الاستثماري.",
        acadTitle: "الشؤون المهنية والأكاديمية",
        acadDesc: "دعم التعاون الأكاديمي، التدريب، والتطوير المهني."
      },
      services: {
        title: "خدماتنا",
        subtitle: "مسارات خدمية متكاملة تغطي التسجيل وإدارة الوفود وتطوير الشراكات.",
        viewAll: "استعرض كل الخدمات",
        items: [
          "إقامة المعارض الدولية",
          "تنسيق اللقاءات الثنائية والمؤسسية",
          "التدريب والتطوير",
          "تنظيم الفعاليات والتمثيل التجاري",
          "تطوير الشراكات",
          "دعم دخول السوق",
          "التنسيق والاتصال بين الحكومة والشركات"
        ]
      },
      events: {
        title: "الفعاليات الدولية البارزة",
        subtitle: "مجموعة مختارة من المعارض والقمم والمنتديات الدولية القادمة.",
        viewAll: "استعرض كل الفعاليات"
      },
      process: {
        title: "عملية الشراكة لدينا",
        subtitle: "تضمن عمليتنا المجربة بناء علاقات هادفة وتحقيق نتائج ملموسة.",
        discoverTitle: "اكتشاف",
        discoverDesc: "نفهم أهدافك واحتياجاتك.",
        connectTitle: "تواصل",
        connectDesc: "نربطك بالفرص والشركاء المناسبين.",
        collaborateTitle: "تعاون",
        collaborateDesc: "نسهل المشاركة والاندماج وبناء الشراكات.",
        growTitle: "نمو",
        growDesc: "ندعم الأثر طويل الأمد والنجاح المستدام."
      },
      partners: {
        title: "شركاء يثقون بنا",
        subtitle: "وزارات وغرف ومؤسسات وطنية تعمل مع منصة جاز."
      },
      news: {
        title: "الأخبار والآراء",
        viewAll: "استعرض كل المقالات"
      }
    },
    // Events
    events: {
      limitedSeats: "مقاعد محدودة",
      openRegistration: "التسجيل مفتوح",
      title: "الفعاليات والمعارض",
      subtitle: "اكتشف جميع الفعاليات والمعارض القادمة وسجل حضورك الآن",
      upcoming: "الفعاليات القادمة",
      upcomingSubtitle: "اكتشف أحدث المعارض والفعاليات المقبلة",
      viewAll: "عرض جميع الفعاليات",
      viewDetails: "عرض التفاصيل",
      noEvents: "لا توجد فعاليات متاحة حالياً",
      featured: "مميز",
      happening: "جاري الآن",
      register: "سجل الآن",
      free: "مجاني",
      search: "ابحث عن فعالية...",
      filter: "بحث",
      all: "الكل",
      date: "التاريخ والوقت",
      location: "الموقع",
      capacity: "السعة",
      endDate: "تاريخ الانتهاء",
      price: "السعر",
      person: "شخص",
      aboutEvent: "نبذة عن الفعالية",
      noDescription: "لا يوجد وصف متاح لهذه الفعالية.",
      international: "دولية",
      local: "محلية",
      featuredBadge: "مميزة",
      currency: "د.ع",
      breadcrumbHome: "الرئيسية",
      breadcrumbEvents: "الفعاليات",
      listNoMatchTitle: "لم يتم العثور على فعاليات مطابقة",
      listNoMatchDescription:
        "جرّب تغيير قطاع البحث أو عبارة البحث. نُبقي هذه الصفحة نشطة ليتمكن الزوار من الوصول إلى الفعاليات فور نشرها.",
      listEmptyTitle: "سيتم الإعلان عن الفعاليات القادمة قريباً",
      listEmptyDescription:
        "يمكنك الإبقاء على هذه الصفحة متاحة أثناء تجهيز التقويم؛ ذلك يطمئن الزوار بأن القسم موجود ويقدّم لهم خطوات واضحة بدلاً من صفحة فارغة.",
      exploreSectors: "استكشف أقسامنا الاستراتيجية",
      availableSectors: "الأقسام الاستراتيجية المتاحة",
      filterResults: "تصفية النتائج",
      sectorCategoriesLabel: "فئات الأقسام الاستراتيجية",
      searchPlaceholderDetail: "ابحث باسم الفعالية أو الكلمات المفتاحية...",
      stats: {
        total: { label: "فعالية إجمالاً", suffix: "" },
        upcoming: { label: "فعالية قادمة", suffix: "" },
        countries: { label: "دولة شريكة", suffix: "+" },
        divisions: { label: "قسم استراتيجي", suffix: "" },
      },
      contactBanner: {
        title: "تريد تنظيم فعالية معنا؟",
        description:
          "نساعد المنظمين والرعاة والشركاء في الوصول إلى صنّاع القرار عبر أقسامنا الاستراتيجية.",
        cta: "تواصل معنا",
      },
    },
    // Event Tabs / Conference Sections
    eventTabs: {
      home: "الصفحة الرئيسية",
      theme: "موضوع المؤتمر",
      sponsors: "الرعاة",
      exhibitors: "العارضون",
      partners: "الشركاء",
      registration: "التسجيل",
      program: "البرنامج",
      submissionForm: "نموذج التقديم",
      submitRequest: "إرسال طلب",
      noContent: "لا يوجد محتوى لهذا القسم حالياً",
      registerNow: "سجّل الآن",
      dontMissOut: "Don't Miss Out",
      registrationOpen: "التسجيل مفتوح الآن",
      registerSubtitle: "سجّل الآن واحجز مقعدك. المقاعد محدودة!",
    },
    // Sectors
    sectors: {
      title: "أقسامنا الاستراتيجية",
      subtitle:
        "نغطي مجموعة من الأقسام الاستراتيجية التي تساهم في تطوير الاقتصاد العراقي",
      learnMore: "اكتشف المزيد",
      activeEvents: "فعالية نشطة",
      contactBanner: {
        title: "هل أنت مهتم بالتعاون معنا؟",
        description:
          "نحن نعمل عبر قطاعات متعددة لدعم النمو وبناء شراكات مثمرة. تواصل معنا اليوم لنستكشف فرص التعاون معاً.",
        cta: "تواصل معنا",
      },
      stats: {
        years: { label: "سنوات من الخبرة", suffix: "+" },
        events: { label: "فعالية مُنظَّمة", suffix: "+" },
        countries: { label: "دولة شريكة", suffix: "+" },
        divisions: { label: "أقسام استراتيجية", suffix: "" },
      },
      methodology: {
        eyebrow: "المنهجية",
        title: "لماذا هذه الأقسام الأربعة؟",
        subtitle:
          "اختيرت أقسامنا الأربعة بناءً على معطيات الاقتصاد العراقي وطبيعة الطلب المحلي والشراكات الدولية المتاحة، لتشكل معاً منظومة متكاملة تخدم التنمية المستدامة.",
        reasons: {
          medical: {
            title: "الرعاية الصحية",
            description:
              "القطاع الأكثر حيويةً في إعادة بناء البنية السريرية، مع شراكات طبية دولية مباشرة.",
          },
          technology: {
            title: "التحول الرقمي",
            description:
              "ركيزة تسريع جاهزية المؤسسات الوطنية وتبني حلول تقنية متقدمة وآمنة.",
          },
          academia: {
            title: "الشؤون الأكاديمية",
            description:
              "بناء جسور بحثية وتعليمية بين الجامعات العراقية ومراكز التميز العالمية.",
          },
          industrie: {
            title: "التطوير الصناعي",
            description:
              "توسيع آفاق التجارة وربط الصناعة المحلية بمراكز التصنيع والأسواق الدولية.",
          },
        },
      },
    },
    // Blog listing
    blogPage: {
      badge: "تحديثات مستمرة",
      kicker: "بيانات جاز",
      title: "الأخبار",
      subtitle:
        "اطلع على أحدث المقالات والرؤى حول عالم الفعاليات والمعارض وتطورات الأعمال في العراق.",
      edition: "العدد",
      articlesCount: "مقال",
      established: "منذ 2022",
      featured: "مقال مميز",
      readArticle: "اقرأ المقال",
      searchPlaceholder: "ابحث في المقالات...",
      searchLabel: "بحث",
      categoriesLabel: "التصنيفات",
      allCategories: "الكل",
      noResultsTitle: "لا توجد نتائج",
      noResultsDescription: "لم نعثر على مقالات تطابق بحثك. جرّب كلمات مختلفة أو أعد ضبط الفلاتر.",
      clearFilters: "إعادة الضبط",
      emptyTitle: "الرؤى الاستراتيجية والتحليلات الاقتصادية",
      emptyDescription: "نعمل حالياً على إعداد تقارير تحليلية متكاملة ودراسات قطاعية متخصصة تغطي بيئة الأعمال والاستثمار في العراق. تصفح دليل الفعاليات القائم للتواصل المباشر مع القطاعات النشطة.",
      readingMinutes: "دقيقة",
      pagination: {
        previous: "السابق",
        next: "التالي",
        page: "صفحة",
        of: "من",
      },
      stats: {
        articles: { label: "مقال منشور", suffix: "" },
        categories: { label: "تصنيف", suffix: "" },
        readingTime: { label: "دقيقة قراءة", suffix: "" },
        years: { label: "سنوات تغطية", suffix: "+" },
      },
      contactBanner: {
        title: "تبحث عن تقرير مخصص؟",
        description:
          "فريقنا التحريري يُعدّ تقارير قطاعية وجلسات إحاطة مخصصة حسب الطلب.",
        cta: "اطلب إحاطة",
      },
    },
    // CTA
    cta: {
      title: "هل لديك فعالية قادمة؟",
      subtitle:
        "تواصل معنا اليوم لنساعدك في تنظيم فعاليتك بأعلى معايير الجودة والاحترافية",
    },
    // Contact
    contact: {
      title: "تواصل معنا",
      subtitle:
        "نحن نثمن التعاون ومستعدون لدعم أهدافك. سواء كنت مهتماً بفرص التعاون، أو مناقشات الشراكة، أو المشاركة في فعالياتنا الدولية، أو بحاجة لدعم للحصول على دعوة رسمية، فإن فريقنا مستعد لمساعدتك.",
      address: "العنوان",
      phone: "الهاتف",
      email: "البريد الإلكتروني",
      workingHours: "ساعات العمل",
      workingDays: "الأحد - الخميس",
      workingTime: "8:30 ص - 4:30 م",
      fullName: "الاسم الكامل",
      subject: "الموضوع",
      message: "الرسالة",
      messagePlaceholder: "اكتب رسالتك هنا...",
      send: "إرسال الرسالة",
      success: "تم إرسال رسالتك بنجاح!",
      successMessage: "سنتواصل معك في أقرب وقت ممكن.",
      sendAnother: "إرسال رسالة أخرى",
      category: "نوع التواصل",
      categories: {
        general: "تواصل عام",
        event: "استفسار عن فعالية",
        sector: "استفسار عن قسم استراتيجي",
        blog: "استفسار عن مدونة",
        service: "طلب خدمة",
      },
      selectEvent: "اختر الفعالية",
      selectSector: "اختر القسم الاستراتيجي",
      selectBlog: "اختر المقال",
      heroDescription: "نحن نثمن التعاون ومستعدون لدعم أهدافك. سواء كنت مهتماً بفرص التعاون، أو مناقشات الشراكة، أو المشاركة في فعالياتنا الدولية، أو بحاجة لدعم للحصول على دعوة رسمية، فإن فريقنا مستعد لمساعدتك.",
      cooperationTitle: "طلب التعاون",
      cooperationDesc: "استكشف فرص التعاون المشترك مع JAZ للمشاريع والمبادرات.",
      invitationTitle: "طلب دعم الدعوات",
      invitationDesc: "طلب خطابات دعوة رسمية للحصول على تأشيرة الدخول، والفعاليات، والمعارض، والاجتماعات المؤسسية.",
      partnerTitle: "كن شريكاً في العراق",
      partnerDesc: "انضم إلى شبكتنا من المؤسسات والمنظمات لتعزيز شراكات مؤثرة.",
      sendUsMessage: "أرسل لنا رسالة",
      formSubtitle: "املأ النموذج أدناه وسيقوم فريقنا بالرد عليك في أقرب وقت ممكن.",
      officesSubtitle: "ثلاث مكاتب تغطي العراق من الشمال إلى الجنوب.",
      organization: "المؤسسة / الشركة",
      organizationPlaceholder: "أدخل اسم مؤسستك أو شركتك",
      agreeTerms: "أوافق على سياسة الخصوصية والشروط والأحكام.",
      ourOffices: "مكاتبنا في العراق",
      getDirections: "احصل على الاتجاهات",
      basraOffice: "مكتب البصرة",
      basraAddress: "مبنى شركة نفط وغاز البصرة\nشارع الكورنيش، البصرة\nالعراق",
      baghdadOffice: "مكتب بغداد",
      baghdadAddress: "حي المنصور\nشارع النضال، بغداد\nالعراق",
      erbilOffice: "مكتب أربيل",
      erbilAddress: "مجمع معرض أربيل الدولي\nشارع 60م، أربيل\nالعراق",
      otherWaysTitle: "طرق أخرى للتواصل معنا",
      otherWaysSubtitle: "هاتف، بريد إلكتروني، أو لينكد إن — اختر ما يناسبك.",
      callUs: "اتصل بنا",
      callUsDesc: "الأحد-الخميس: 8:30 ص - 4:30 م",
      emailUs: "راسلنا",
      emailUsDesc: "نرد خلال 24 ساعة.",
      visitWebsite: "قم بزيارة موقعنا",
      visitWebsiteDesc: "استكشف خدماتنا وفعالياتنا.",
      connectLinkedIn: "تواصل على لينكد إن",
      connectLinkedInDesc: "تابعنا للحصول على آخر التحديثات.",
      followFacebook: "تابعنا على فيسبوك",
      followFacebookDesc: "ابقَ على تواصل مع مجتمعنا.",
      ourLocation: "موقعنا",
      ourLocationSubtitle: "مكتب البصرة الرئيسي على الخريطة.",
      allOffices: "جميع المكاتب",
    },
    // About
    about: {
      title: "من نحن",
      sectionTitle: "من نحن",
      sectionSubtitle: "بوابتك إلى العالم الدولي للمعارض والمؤتمرات",
      heroTitle:
        "Joint Annual Zone: بوابتك إلى العالم الدولي للمعارض والمؤتمرات",
      description:
        "منذ أكثر من عقد، تعمل Joint Annual Zone (JAZ) كبوابة استراتيجية تربط القطاع المهني في العراق بالمعارض والمؤتمرات والشبكات الدولية ذات الأثر الحقيقي.",
      aboutLabel: "نظرة عامة",
      longDescription:
        "شركة الجاز: منظم دولي للمعارض والمؤتمرات يعتمد استراتيجي وطني ودولي، لأكثر من عشر سنوات، عملنا كبوابة تربط قطاع الأعمال والمهنيين في العراق بأهم الفعاليات الدولية المؤثرة.",
      mission: "رسالتنا",
      missionText:
        "تنسيق المساحات الاستراتيجية التي يلتقي فيها المهنيون والشركات، وتمكينهم من خلق فرص نمو حقيقية.",
      vision: "رؤيتنا",
      visionText:
        "أن نكون البوابة الأكثر ثقة في العراق نحو المعارض والمؤتمرات الدولية.",
      partnerTitle: "لماذا الشراكة مع JAZ؟",
      partnerText:
        "نربط المهنيين العراقيين بالفرص الدولية، ونُيسّر تفاعلات نوعية تدفع التقدم والنجاح.",
      pillarsTitle: "مرتكزاتنا الاستراتيجية",
      pillarsDescription:
        "يلتزم نهجنا بثلاث ركائز أساسية تشكل أساس التميز الذي نقدمه لعملائنا وشركائنا.",
      pillarSuccessTitle: "تحقيق النجاح",
      pillarSuccessText:
        "نقيس أثرنا بمدى نجاح شركائنا، ونرافقهم بخطوات عملية نحو نتائج واضحة وقابلة للقياس.",
      pillarRelationshipsTitle: "بناء العلاقات",
      pillarRelationshipsText:
        "نبني شبكات وشراكات طويلة الأمد تمنح عملاءنا قيمة مستمرة وحضوراً أقوى في الأسواق المستهدفة.",
      pillarOpportunitiesTitle: "خلق الفرص",
      pillarOpportunitiesText:
        "نحوّل العلاقات والفرص الدولية إلى مسارات عملية للنمو والتوسع.",
      servicesTitle: "خدماتنا",
      servicesHeading: "خدمات مختارة للمؤسسات وصنّاع القرار",
      servicesDescription:
        "نقدم حلولاً عملية للمؤسسات والمهنيين الباحثين عن حضور دولي مؤثر وشراكات ذات قيمة.",
      matchmakingTitle: "الربط المهني والشراكات المؤسسية",
      matchmakingText:
        "نصل الشركات والمهنيين العراقيين بشبكات دولية موثوقة لفتح فرص تعاون ونمو ملموسة.",
      trainingTitle: "التدريب والتطوير",
      trainingText:
        "نقدّم برامج تدريب وتطوير تعزز الجاهزية المهنية والقدرات القيادية وفق احتياجات السوق.",
      ourServicesTitle: "خدماتنا",
      ourServicesSubtitle: "ننظم معارض ومؤتمرات دولية، نربط الشركات بالفرص.",
      serviceEventMgmtTitle: "إدارة الفعاليات",
      serviceEventMgmtText:
        "إدارة احترافية للمعارض والمؤتمرات لضمان تجربة سلسة لجميع المشاركين.",
      serviceExhibitionTitle: "تخطيط المعارض",
      serviceExhibitionText:
        "خدمات تخطيط شاملة للمعارض الناجحة، مصممة لتلبية احتياجات أعمالك.",
      serviceCustomerTitle: "دعم العملاء",
      serviceCustomerText:
        "التسجيل في الفعاليات، ومشاهدة التفاصيل، والتواصل معنا للحصول على مساعدة شخصية.",
      whyTitle: "ما الذي يميز JAZ؟",
      whyText:
        "لأننا نجمع بين الفهم المحلي والوصول الدولي، ونحوّل التواصل المهني إلى نتائج عملية.",
      websiteLabel: "الموقع الإلكتروني",
      contactEmail: "contact@jaz.iq",
      contactPhone: "07719000600",
      contactWebsite: "www.jaz.iq",
      ourStory: "قصتنا",
      stats: {
        exhibitions: "معرض ناجح",
        visitors: "زائر",
        partners: "شريك",
        experience: "سنوات خبرة",
      },
      whoWeAreTitle: "من نحن",
      whoWeAreSubtitle: "منصة عراقية تربط الحكومة والشركات والأكاديميا بالمعارض والشركاء الدوليين.",
      whoWeAreText: "جاز (المنطقة السنوية المشتركة) هي منصة عراقية مخصصة تربط الحكومة والشركات والأكاديميا والمهنيين بالمعارض والمؤتمرات الدولية والشركاء المؤسسيين. نحن نسهل المشاركة وننظم الوفود ونبني تعاوناً طويل الأمد يقود النمو المستدام والأثر العالمي.",
      whereWeOperate: "أين نعمل",
      whereWeOperateSubtitle: "ثلاث مكاتب استراتيجية تغطي العراق من الشمال إلى الجنوب.",
      erbilTitle: "أربيل",
      erbilDesc: "خدمة إقليم كردستان والمحافظات الشمالية.",
      baghdadTitle: "بغداد",
      baghdadDesc: "ربط قلب العراق بالفرص العالمية.",
      basraTitle: "البصرة",
      basraDesc: "بوابتنا إلى الخليج والتجارة الدولية.",
      whyJaz: "لماذا جاز",
      whyJazSubtitle: "خمس مزايا تجعل التعاون مع منصة جاز قراراً موثوقاً.",
      whyJazItems: [
        "تنسيق رسمي مع الجهات الحكومية.",
        "الوصول إلى المعارض العالمية.",
        "دعم مخصص للوفود والمشاركات.",
        "تسهيل الشراكات الاستراتيجية.",
        "دعم شامل من البداية للنهاية."
      ],
      ourValues: "قيمنا",
      ourValuesSubtitle: "خمسة مبادئ توجّه كل شراكة ومشاركة نقوم بها.",
      valuesItems: [
        { title: "النزاهة", desc: "نتصرف بصدق وشفافية." },
        { title: "التميز", desc: "نسعى لتحقيق الجودة دائماً." },
        { title: "التعاون", desc: "الشراكات تخلق تأثيراً أعظم." },
        { title: "الابتكار", desc: "نتبنى حلولاً جديدة." },
        { title: "الأثر", desc: "مدفوعون بنتائج ملموسة." }
      ],
      leadership: "القيادة والحضور",
      leadershipText: "يقع مقرنا الرئيسي في البصرة، مع مكاتب في بغداد وأربيل. نعمل وثيقاً مع الوزارات والجامعات وقادة القطاع الخاص.",
      nationwideReach: "الانتشار على مستوى الدولة",
      reachItems: [
        { count: "3 مكاتب استراتيجية", label: "البصرة • بغداد • أربيل" },
        { count: "20+ مؤسسة شريكة", label: "حكومي وأكاديمي وخاص" },
        { count: "50+ شريك دولي", label: "منظمات وسفارات" },
        { count: "100+ وفد مدعوم", label: "تمثيل العراق عالمياً" }
      ],
      ceoSectionLabel: "كلمة القيادة",
      ceoName: "أ. أحمد جاسم",
      ceoTitle: "الرئيس التنفيذي، جاز",
      ceoQuote: "\"لقد بنينا جاز على اقتناع راسخ: أن العراق يستحق حضوراً دولياً حقيقياً. كل معرض ندخله، وكل شراكة نُبرمها، وكل وفد ندعمه — هو خطوة نحو مستقبل يكون فيه المهنيون العراقيون شركاء حقيقيين في صنع الاقتصاد العالمي.\"",
      ceoTagline: "١٠+ سنوات من تحويل الطموح إلى تأثير دائم",
      ceoStat1Val: "١٠+",
      ceoStat1Label: "سنوات خبرة",
      ceoStat2Val: "٥٠+",
      ceoStat2Label: "شريك دولي",
      ceoStat3Val: "٢٥٠+",
      ceoStat3Label: "مبادرة أشرف عليها",
      ctaTitle: "دعونا نبني علاقات أقوى للعراق.",
      ctaDesc: "شارك مع جاز للوصول إلى المنصات العالمية وخلق تأثير دائم.",
      ctaCooperation: "طلب التعاون",
      ctaContact: "اتصل بنا",
      statPartnerCountriesVal: "15",
      statPartnerCountriesLabel: "دولة شريكة",
      statActivePartnersVal: "120",
      statActivePartnersLabel: "شريك نشط",
      statJointInitiativesVal: "250",
      statJointInitiativesLabel: "مبادرة مشتركة",
      statYouthBeneficiariesVal: "1000",
      statYouthBeneficiariesLabel: "شاب مستفيد"
    },
    servicesPage: {
      hero: {
        title: "خدماتنا",
        description: "تقوم جاز بتنسيق وتسهيل التبادل التجاري الثنائي، والوفود الطبية، والشراكات الدولية الاستراتيجية لربط العراق بأحدث المنصات والفرص العالمية.",
        ctaCooperation: "طلب التعاون",
        ctaParticipation: "استكشاف المشاركات",
        ctaInvitation: "طلب دعم الدعوات"
      },
      stats: {
        expVal: "+10",
        expLabel: "سنوات من التنسيق",
        citiesVal: "البصرة • بغداد • أربيل",
        citiesLabel: "مكاتب العراق",
        sectorsVal: "قطاعات متعددة",
        sectorsLabel: "صناعات نشطة",
        govVal: "مؤسساتي",
        govLabel: "شراكات قنصلية"
      },
      coreServices: {
        title: "ركائز الخدمات",
        subtitle: "أربعة مسارات خدمية تغطي الوصول للفعاليات، التحالفات المؤسسية، التمثيل الميداني، واللوجستيات القنصلية.",
        items: [
          {
            title: "إقامة المعارض الدولية",
            desc: "تنظيم وإقامة المعارض التجارية الدولية والمشاركة المتكاملة فيها، بما يشمل إدارة الوفود الرسمية بالكامل — من التسجيل وتأشيرات السفر إلى التنسيق القنصلي."
          },
          {
            title: "تنسيق اللقاءات الثنائية والمؤسسية",
            desc: "بناء شراكات رسمية، ومشاريع مشتركة، واتفاقيات نقل المعرفة والتكنولوجيا بين الشركات العالمية والوزارات والجهات المطورة محلياً."
          },
          {
            title: "التمثيل الميداني ودخول السوق العراقي",
            desc: "تقديم خدمات التمثيل الميداني والدعم الاستراتيجي لدخول السوق، وربط المنظمات الدولية بالوزارات المعنية وغرف التجارة في العراق."
          },
          {
            title: "اللوجستيات القنصلية والتغطية الإعلامية",
            desc: "إدارة خدمات البروتوكول اللوجستي لكبار الشخصيات، وتأشيرات السفر، والتغطية الإعلامية واسعة الانتشار لتعزيز أثر المشاركات."
          }
        ]
      },
      info: {
        howWeWork: {
          title: "كيف نعمل",
          desc: "من خلال استراتيجيات مخصصة للتمثيل الدولي."
        },
        industries: {
          title: "قطاعات ندعمها",
          desc: "الرعاية الصحية، التكنولوجيا، التطوير التجاري والأكاديمي."
        },
        whyJaz: {
          title: "لماذا جاز؟",
          items: [
            "علاقات دولية ومحلية موثوقة.",
            "سجل حافل من المشاركات الناجحة."
          ]
        }
      },
      cta: {
        title: "لنصنع فرصاً عالمية معاً",
        description: "سواء كنت تمثل مؤسسة أو شركة تجارية، جاز هي شريكك للتواصل الدولي والأثر الملموس.",
        cooperation: "تواصل معنا للتعاون",
        contact: "اتصل بنا"
      }
    },
    // Auth
    auth: {
      login: "تسجيل الدخول",
      register: "إنشاء حساب جديد",
      logout: "تسجيل الخروج",
      email: "البريد الإلكتروني",
      password: "كلمة المرور",
      confirmPassword: "تأكيد كلمة المرور",
      fullName: "الاسم الكامل",
      phone: "رقم الهاتف",
      loginSubtitle: "أدخل بياناتك للوصول إلى حسابك",
      registerSubtitle: "سجل الآن للمشاركة في الفعاليات",
      noAccount: "ليس لديك حساب؟",
      hasAccount: "لديك حساب بالفعل؟",
      createAccount: "إنشاء الحساب",
      invalidCredentials: "البريد الإلكتروني أو كلمة المرور غير صحيحة",
      passwordMismatch: "كلمات المرور غير متطابقة",
      passwordTooShort: "كلمة المرور يجب أن تكون 6 أحرف على الأقل",
      verifyEmail: "تحقق من بريدك الإلكتروني",
      verifyEmailMessage:
        "لقد أرسلنا رابط التحقق إلى بريدك الإلكتروني. يرجى النقر على الرابط لتفعيل حسابك.",
      backToLogin: "العودة لتسجيل الدخول",
    },
    // Registration
    registration: {
      title: "تأكيد التسجيل",
      success: "تم التسجيل بنجاح!",
      congratulations: "مبروك! تم تسجيلك في الفعالية",
      ticketNumber: "رقم التذكرة الخاص بك:",
      keepTicket: "احتفظ برقم التذكرة لتقديمه عند الدخول",
      browseOther: "تصفح فعاليات أخرى",
      registerAs: "سيتم التسجيل باسم:",
      confirm: "تأكيد التسجيل",
      backToEvent: "العودة للفعالية",
      error: "حدث خطأ أثناء التسجيل. يرجى المحاولة مرة أخرى.",
      loading: "جاري التحميل...",
    },
    // Footer
    footer: {
      description:
        "منصة متكاملة لتنظيم الفعاليات، المعارض، وبرامج التدريب والتطوير في العراق.",
      company: "الشركة",
      team: "فريق العمل",
      partners: "الشركاء",
      careers: "الوظائف",
      services: "خدماتنا",
      exhibitions: "المعارض",
      support: "الدعم",
      faq: "الأسئلة الشائعة",
      privacy: "سياسة الخصوصية",
      terms: "الشروط والأحكام",
      rights: "جميع الحقوق محفوظة.",
      location: "بغداد، المنصور، شارع 14 رمضان",
    },
    // Dashboard
    dashboard: {
      users: "المستخدمين",
      registrations: "التسجيلات",
      settings: "الإعدادات",
      sectorRegistrations: "تسجيلات القطاعات",
      messages: "الرسائل",
    },
    // Common
    common: {
      loading: "جاري التحميل...",
      error: "حدث خطأ",
      save: "حفظ",
      cancel: "إلغاء",
      delete: "حذف",
      edit: "تعديل",
      add: "إضافة",
      search: "بحث",
      day: "يوم",
      hour: "ساعة",
      minute: "دقيقة",
      home: "الرئيسية",
    },
    // Training Page
    trainingPage: {
      badge: "مركز التدريب والتطوير",
      title: "التدريب والتطوير",
      subtitle: "برامج تدريبية متخصصة قريباً",
      features: {
        experts: {
          title: "خبراء معتمدون دولياً",
          description: "خبرة عملية ونقل حقيقي للمهارات",
        },
        certificates: {
          title: "شهادات مهنية معتمدة",
          description: "اعتمادات معترف بها محلياً ودولياً",
        },
        interactive: {
          title: "منهجية تطبيقية تفاعلية",
          description: "تركيز كامل على دراسة الحالات والتطبيق المباشر",
        },
        flexible: {
          title: "مرونة كاملة في الوقت",
          description: "جداول مرنة تناسب احتياجات الجهات المشاركة",
        },
      },
      programsTitle: "البرامج التدريبية المتاحة",
      instructor: "المدرب",
      free: "مجاني",
      currency: "د.ع",
      defaultDescription: "برنامج تدريبي متخصص",
      emptyTitle: "تطوير الكفاءات والقيادات التنفيذية",
      emptyDescription: "نعمل حالياً على تصميم وتجهيز باقة متكاملة من البرامج التدريبية المتقدمة لعام ٢٠٢٦، بالتعاون مع نخبة من الخبراء الدوليين والمحليين. يمكنك التسجيل المسبق الآن لإخطارك فور إطلاق البرامج وحجز مقعدك.",
      emptyButton: "تواصل مع مستشار التدريب",
      ctaTitle: "هل تحتاج برنامج تدريبي مخصص؟",
      ctaDescription: "برامج حسب احتياجكم",
      ctaButton: "تواصل معنا",
    },
    // Time units
    time: {
      day: "يوم",
      hour: "ساعة",
      minute: "دقيقة",
    },
  },
  en: {
    // Navigation
    nav: {
      home: "Home",
      services: "Services",
      about: "About Us",
      events: "Events",
      sectors: "Departments",
      blog: "Blog",
      training: "Training",
      contact: "Contact Us",
      partners: "Partnership",
      links: "Links",
      calendar: "Calendar",
      invitationSupport: "Invitation Support",
    },
    // Partners Page
    partnersPage: {
      hero: {
        title: "Our Partners",
        subtitle:
          "Together towards a sustainable future! Join our network of partners and contribute to making a change.",
      },
      corporate: {
        title: "Corporate Partnerships: Sustainable Impact for Your Brand",
        description:
          "Make a difference in the world by collaborating with us to promote environmental and social responsibility, attract conscious customers, and contribute to achieving sustainable development goals.",
        types: {
          commercial: {
            title: "Commercial Partnerships",
            description:
              "Support environmentally friendly products and services or finance sustainable projects.",
          },
          marketing: {
            title: "Marketing Partnerships",
            description:
              "Joint promotion of sustainable initiatives across social media platforms.",
          },
          technical: {
            title: "Technical Support",
            description:
              "Provide innovative technology solutions, such as improving energy efficiency and analyzing environmental data.",
          },
          social: {
            title: "Social Responsibility",
            description:
              "Sponsor awareness campaigns and community training programs.",
          },
        },
        contact: "Contact us via email or fill out the collaboration form.",
        cta: "Join Our Partner Network Today!",
      },
      volunteers: {
        title: "Volunteer Energies: Be Part of the Solution!",
        description:
          "Contribute to environmental and social change, gain valuable experience, and be part of a community that cares about sustainability.",
        roles: {
          graphic: {
            title: "Graphic Design",
            description:
              "Create visual content to support awareness campaigns.",
          },
          management: {
            title: "Project Management",
            description:
              "Organize events and follow up on sustainable initiatives.",
          },
          marketing: {
            title: "Digital Marketing",
            description: "Manage social media platforms and write content.",
          },
          community: {
            title: "Community Support",
            description:
              "Train individuals and participate in field activities.",
          },
          dev: {
            title: "Web Development",
            description: "Improve the website and develop green digital tools.",
          },
        },
        cta: "Register as a Volunteer Now!",
      },
      dialog: {
        partner: {
          title: "Start Partnership With Us",
          description:
            "Choose your preferred method to contact us and begin the partnership journey.",
        },
        volunteer: {
          title: "Join Our Volunteer Team",
          description:
            "Choose the method that suits you best to register and join us.",
        },
        ideas: {
          title: "Share Your Idea With Us",
          description:
            "Choose your preferred method to share your idea and we will contact you.",
        },
        email: "Send Email",
        form: "Fill Form",
      },
      ideas: {
        title:
          "Have an idea for a sustainable project? We are here to support you!",
        description:
          "We help individuals and institutions turn ideas into real projects that serve the environment and society.",
        cta: "Share your idea with us now, and we'll discuss how to support and develop it!",
      },
    },
    // Calendar Page
    calendarPage: {
      title: "Events Calendar",
      subtitle:
        "Explore the schedule of upcoming events and exhibitions throughout the year",
      search: "Search for an event...",
      filterCategory: "Category",
      filterLocation: "Location",
      viewPast: "View past events",
      allCategories: "All Categories",
      allLocations: "All Locations",
    },
    // Hero Section
    hero: {
      badge: "The First Events Platform in Iraq",
      title: "JAZ",
      subtitle: "Joint Annual Zone to Your Place",
      description:
        "Your Strategic Gateway to International Exhibitions and Conferences.",
      browseEvents: "Browse Upcoming Events",
      contactUs: "Contact Us",
      stats: {
        exhibitions: "Successful Exhibitions",
        visitors: "Visitors",
        experience: "Years of Experience",
      },
    },
    homepage: {
      hero: {
        title: "Iraq's Gateway to Global Exhibitions & Partnerships",
        description: "Connecting institutions with international exhibitions and sustainable partnerships.",
        ctaCooperation: "Request Cooperation",
        ctaParticipation: "International Participation",
        ctaPartner: "Partner in Iraq",
        ctaInvitation: "Invitation Support",
        statPartnerCountriesVal: "15",
        statPartnerCountriesLabel: "Partner Countries",
        statActivePartnersVal: "120",
        statActivePartnersLabel: "Active Partners",
        statJointInitiativesVal: "250",
        statJointInitiativesLabel: "Joint Initiatives",
        statYouthBeneficiariesVal: "1000",
        statYouthBeneficiariesLabel: "Youth Beneficiaries"
      },
      sectors: {
        title: "Our Focus Sectors",
        subtitle: "Four domains we focus on to connect Iraqi institutions with global markets.",
        viewAll: "Browse all sectors",
        medicalTitle: "Healthcare & Life Sciences",
        medicalDesc: "Connecting health institutions with global innovation and medical advancements.",
        techTitle: "Digital Transformation & Technology",
        techDesc: "Empowering Iraq's digital future through global tech exhibitions and partnerships.",
        indTitle: "Industrial & Commercial Development",
        indDesc: "Advancing industrial growth, trade opportunities, and investment cooperation.",
        acadTitle: "Professional & Academic Affairs",
        acadDesc: "Supporting academic collaboration, training, and professional development."
      },
      services: {
        title: "Our Services",
        subtitle: "Integrated service tracks covering registration, delegation management, and partnership development.",
        viewAll: "Browse all services",
        items: [
          "International Exhibition Participation",
          "B2B & Institutional Matchmaking",
          "Training & Development",
          "Event Organization & Representation",
          "Partnership Development",
          "Market Entry Support",
          "Government & Business Liaison"
        ]
      },
      events: {
        title: "Featured International Events",
        subtitle: "A curated selection of upcoming international exhibitions, summits, and forums.",
        viewAll: "Browse all events"
      },
      process: {
        title: "Our Partnership Process",
        subtitle: "Our proven process ensures meaningful connections and measurable outcomes.",
        discoverTitle: "Discover",
        discoverDesc: "We understand your goals and needs.",
        connectTitle: "Connect",
        connectDesc: "We connect you with the right opportunities and partners.",
        collaborateTitle: "Collaborate",
        collaborateDesc: "We facilitate engagement, participation, and partnerships.",
        growTitle: "Grow",
        growDesc: "We support long-term impact and sustainable success."
      },
      partners: {
        title: "Trusted by Institutions & Partners",
        subtitle: "Ministries, chambers, and national institutions working with JAZ."
      },
      news: {
        title: "News & Insights",
        viewAll: "Browse all articles"
      }
    },
    // Events
    events: {
      limitedSeats: "Limited Seats",
      openRegistration: "Open Registration",
      title: "Events and Exhibitions",
      subtitle: "Discover all upcoming events and exhibitions and register now",
      upcoming: "Upcoming Events",
      upcomingSubtitle: "Discover the latest upcoming exhibitions and events",
      viewAll: "View All Events",
      viewDetails: "View Details",
      noEvents: "No events available at the moment",
      featured: "Featured",
      happening: "Happening Now",
      register: "Register Now",
      free: "Free",
      search: "Search for an event...",
      filter: "Search",
      all: "All",
      date: "Date and Time",
      location: "Location",
      capacity: "Capacity",
      endDate: "End Date",
      price: "Price",
      person: "person",
      aboutEvent: "About the Event",
      noDescription: "No description available for this event.",
      international: "International",
      local: "Local",
      featuredBadge: "Featured",
      currency: "IQD",
      breadcrumbHome: "Home",
      breadcrumbEvents: "Events",
      listNoMatchTitle: "No matching events were found",
      listNoMatchDescription:
        "Try changing the selected sector or search term. We are keeping this page active so visitors can find future events as soon as they are published.",
      listEmptyTitle: "Upcoming events will be announced soon",
      listEmptyDescription:
        "You can keep this page live while your calendar is being prepared. It reassures visitors that the section exists and gives them clear next steps instead of an empty page.",
      exploreSectors: "Explore Our Strategic Divisions",
      availableSectors: "Available strategic divisions",
      filterResults: "Filter Results",
      sectorCategoriesLabel: "Strategic Division Categories",
      searchPlaceholderDetail: "Search for event name or keywords...",
      stats: {
        total: { label: "Total Events", suffix: "" },
        upcoming: { label: "Upcoming Events", suffix: "" },
        countries: { label: "Partner Countries", suffix: "+" },
        divisions: { label: "Strategic Divisions", suffix: "" },
      },
      contactBanner: {
        title: "Want to organize an event with us?",
        description:
          "We help organizers, sponsors, and partners reach decision-makers across our strategic divisions.",
        cta: "Contact Us",
      },
    },
    // Event Tabs / Conference Sections
    eventTabs: {
      home: "Home",
      theme: "Conference Theme",
      sponsors: "Sponsors",
      exhibitors: "Exhibitors",
      partners: "Partners",
      registration: "Registration",
      program: "Program",
      submissionForm: "Submission Form",
      submitRequest: "Submit Request",
      noContent: "No content available for this section",
      registerNow: "Register Now",
      dontMissOut: "Don't Miss Out",
      registrationOpen: "Registration is Now Open",
      registerSubtitle:
        "Register now and reserve your seat. Limited seats available!",
    },
    // Sectors
    sectors: {
      title: "Our Strategic Divisions",
      subtitle:
        "We cover strategic divisions that contribute to the development of the Iraqi economy",
      learnMore: "Learn More",
      activeEvents: "active events",
      contactBanner: {
        title: "Interested in working with us?",
        description:
          "We operate across multiple sectors to support growth and build meaningful partnerships. Reach out today and let's explore opportunities together.",
        cta: "Contact Us",
      },
      stats: {
        years: { label: "Years of Expertise", suffix: "+" },
        events: { label: "Events Organized", suffix: "+" },
        countries: { label: "Partner Countries", suffix: "+" },
        divisions: { label: "Strategic Divisions", suffix: "" },
      },
      methodology: {
        eyebrow: "Methodology",
        title: "Why These Four Divisions?",
        subtitle:
          "Our four divisions were selected based on Iraq's economic landscape, local demand signals, and available international partnerships, forming an integrated ecosystem for sustainable development.",
        reasons: {
          medical: {
            title: "Healthcare",
            description:
              "The most vital sector for rebuilding clinical infrastructure, with direct international medical partnerships.",
          },
          technology: {
            title: "Digital Transformation",
            description:
              "Accelerating national institutional readiness and adopting advanced, secure technology solutions.",
          },
          academia: {
            title: "Academic Affairs",
            description:
              "Building research and educational bridges between Iraqi universities and global centers of excellence.",
          },
          industrie: {
            title: "Industrial Development",
            description:
              "Expanding trade horizons by linking local industry with international manufacturing hubs and markets.",
          },
        },
      },
    },
    // Blog listing
    blogPage: {
      badge: "Continuous Updates",
      kicker: "JAZ Dispatch",
      title: "News",
      subtitle:
        "Discover the latest articles and insights about the world of events, exhibitions, and business developments in Iraq.",
      edition: "Edition",
      articlesCount: "Articles",
      established: "Est. 2022",
      featured: "Featured",
      readArticle: "Read Article",
      searchPlaceholder: "Search articles...",
      searchLabel: "Search",
      categoriesLabel: "Categories",
      allCategories: "All",
      noResultsTitle: "No matching articles",
      noResultsDescription: "We couldn't find any articles matching your search. Try different keywords or clear the filters.",
      clearFilters: "Clear filters",
      emptyTitle: "Strategic Insights & Business Intelligence",
      emptyDescription:
        "We are curating a series of in-depth analysis reports, sector briefings, and economic updates regarding Iraq's business ecosystem. Explore our ongoing events directory to stay connected with active market sectors.",
      readingMinutes: "min",
      pagination: {
        previous: "Previous",
        next: "Next",
        page: "Page",
        of: "of",
      },
      stats: {
        articles: { label: "Published Articles", suffix: "" },
        categories: { label: "Categories", suffix: "" },
        readingTime: { label: "Reading Minutes", suffix: "" },
        years: { label: "Years of Coverage", suffix: "+" },
      },
      contactBanner: {
        title: "Looking for a custom briefing?",
        description:
          "Our editorial team prepares tailored sector reports and market briefings on request.",
        cta: "Request a Briefing",
      },
    },
    // CTA
    cta: {
      title: "Have an upcoming event?",
      subtitle:
        "Contact us today to help you organize your event with the highest quality and professionalism standards",
    },
    // Contact
    contact: {
      title: "Contact Us",
      subtitle:
        "We value collaboration and are here to support your goals. Whether you are interested in cooperation opportunities, partnership discussions, participating in our international events, or need support with official invitations, our team is ready to assist you.",
      address: "Address",
      phone: "Phone",
      email: "Email",
      workingHours: "Working Hours",
      workingDays: "Sunday - Thursday",
      workingTime: "8:30 AM - 4:30 PM",
      fullName: "Full Name",
      subject: "Subject",
      message: "Message",
      messagePlaceholder: "Write your message here...",
      send: "Send Message",
      success: "Your message has been sent successfully!",
      successMessage: "We will contact you as soon as possible.",
      sendAnother: "Send Another Message",
      category: "Contact Category",
      categories: {
        general: "General Inquiry",
        event: "Event Inquiry",
        sector: "Strategic Division Inquiry",
        blog: "Blog Inquiry",
        service: "Service Request",
      },
      selectEvent: "Select Event",
      selectSector: "Select Strategic Division",
      selectBlog: "Select Blog Post",
      heroDescription: "We value collaboration and are here to support your goals. Whether you are interested in cooperation opportunities, partnership discussions, participating in our international events, or need support with official invitations, our team is ready to assist you.",
      cooperationTitle: "Request Cooperation",
      cooperationDesc: "Explore cooperation opportunities with JAZ for projects and initiatives.",
      invitationTitle: "Request Invitation Support",
      invitationDesc: "Request official invitation letters for visa, events, exhibitions, and institutional meetings.",
      partnerTitle: "Become a Partner in Iraq",
      partnerDesc: "Join our network of institutions and organizations advancing impactful partnerships.",
      sendUsMessage: "Send Us a Message",
      formSubtitle: "Fill out the form below and our team will get back to you as soon as possible.",
      officesSubtitle: "Three offices covering Iraq from north to south.",
      organization: "Organization / Company",
      organizationPlaceholder: "Enter your organization or company",
      agreeTerms: "I agree to the Privacy Policy and Terms & Conditions.",
      ourOffices: "Our Offices in Iraq",
      getDirections: "Get Directions",
      basraOffice: "Basra Office",
      basraAddress: "Basra Oil & Gas Company Building\nCorniche Road, Basra\nIraq",
      baghdadOffice: "Baghdad Office",
      baghdadAddress: "Al-Mansour District\nAl-Nidhal Street, Baghdad\nIraq",
      erbilOffice: "Erbil Office",
      erbilAddress: "Erbil International Fair Complex\n60m St, Erbil\nIraq",
      otherWaysTitle: "Other Ways to Reach Us",
      otherWaysSubtitle: "Phone, email, or LinkedIn — pick what suits you.",
      callUs: "Call Us",
      callUsDesc: "Sun-Thu: 8:30 AM - 4:30 PM",
      emailUs: "Email Us",
      emailUsDesc: "We reply within 24 hours.",
      visitWebsite: "Visit Our Website",
      visitWebsiteDesc: "Explore our services & events.",
      connectLinkedIn: "Connect on LinkedIn",
      connectLinkedInDesc: "Follow us for updates.",
      followFacebook: "Follow on Facebook",
      followFacebookDesc: "Stay connected with our community.",
      ourLocation: "Our Location",
      ourLocationSubtitle: "The main Basra office on the map.",
      allOffices: "All Offices",
    },
    // About
    about: {
      title: "About Us",
      sectionTitle: "About Us",
      sectionSubtitle:
        "Your Gateway to the International World of Exhibitions and Conferences",
      heroTitle:
        "Joint Annual Zone: Your Gateway to the International World of Exhibitions and Conferences",
      description:
        "For over a decade, Joint Annual Zone (JAZ) has served as Iraq’s specialized strategic gateway, connecting local ambition with international innovation across trade, technology, medicine, and academia.",
      aboutLabel: "Overview",
      longDescription:
        "Joint Annual Zone (JAZ) is an international exhibition and conference organizer with a strategic network of branches inside and outside Iraq. For over a decade, we have been the gateway for Iraqi professionals and businesses, facilitating access to high-impact international events.",
      mission: "Our Mission",
      missionText:
        "To lead the 'Zone' where professionals and corporations converge.",
      vision: "Our Vision",
      visionText:
        "To be the premier gateway to the international world of exhibitions and conferences.",
      partnerTitle: "Why Partner with JAZ?",
      partnerText:
        "We connect Iraqi professionals with international opportunities, facilitating meaningful interactions that drive progress and success.",
      pillarsTitle: 'Our "ABC" Pillars',
      pillarsDescription:
        "Our commitment to excellence is built on three core pillars that guide every partnership and participation.",
      pillarSuccessTitle: "Achieving Success",
      pillarSuccessText: "We measure our success by our clients' success.",
      pillarRelationshipsTitle: "Building Relationships",
      pillarRelationshipsText:
        "We focus on long-term strategic networks and partnerships.",
      pillarOpportunitiesTitle: "Creating Opportunities",
      pillarOpportunitiesText:
        "We facilitate measurable business opportunities for our clientele.",
      servicesTitle: "What We Do",
      servicesHeading: "Services for Company and Institution founders",
      servicesDescription:
        "Joint Annual Zone provides a curated suite of services designed for decision-makers, position holders, and industry leaders seeking meaningful international engagement.",
      matchmakingTitle: "International Events: Exhibitions and Conferance",
      matchmakingText:
        "We specialize in facilitating high-level access for Iraqi Business Visitors to premier international events.",
      trainingTitle: "Training and Development",
      trainingText:
        "Advance Your Career with Internationally Accredited Training Programs",
      ourServicesTitle: "Our Services",
      ourServicesSubtitle:
        "We organize international trade shows and events, connecting businesses and opportunities.",
      serviceEventMgmtTitle: "Event Management",
      serviceEventMgmtText:
        "Expertly managing exhibitions and conferences to ensure seamless experiences for all participants.",
      serviceExhibitionTitle: "Exhibition Planning",
      serviceExhibitionText:
        "Comprehensive planning services for successful exhibitions, tailored to meet your business needs.",
      serviceCustomerTitle: "Customer Support",
      serviceCustomerText:
        "Register for events, view details, and contact us for personalized assistance.",
      whyTitle: "Why JAZ",
      whyText:
        "We connect Iraqi professionals with international opportunities, facilitating meaningful interactions that drive progress and success.",
      websiteLabel: "Website",
      contactEmail: "contact@jaz.iq",
      contactPhone: "07719000600",
      contactWebsite: "www.jaz.iq",
      ourStory: "Our Story",
      stats: {
        exhibitions: "Successful Exhibitions",
        visitors: "Visitors",
        partners: "Partners",
        experience: "Years of Experience",
      },
      whoWeAreTitle: "Who We Are",
      whoWeAreSubtitle: "Iraq's platform connecting government, business, and academia with international exhibitions and partners.",
      whoWeAreText: "JAZ (Joint Annual Zone) is Iraq's dedicated platform that connects government, businesses, academia, and professionals with international exhibitions, conferences, and institutional partners. We facilitate participation, organize delegations, and build long-term cooperation that drives sustainable growth and global impact.",
      whereWeOperate: "Where We Operate",
      whereWeOperateSubtitle: "Three strategic offices covering Iraq from north to south.",
      erbilTitle: "Erbil",
      erbilDesc: "Serving the Kurdistan region and northern provinces.",
      baghdadTitle: "Baghdad",
      baghdadDesc: "Connecting the heart of Iraq with global opportunities.",
      basraTitle: "Basra",
      basraDesc: "Our gateway to the Gulf and international trade.",
      whyJaz: "Why JAZ",
      whyJazSubtitle: "Five advantages that make partnering with JAZ a trusted choice.",
      whyJazItems: [
        "Official coordination with government agencies.",
        "Direct access to global exhibitions.",
        "Dedicated delegation and participation support.",
        "Facilitation of strategic partnerships.",
        "End-to-end comprehensive coordination."
      ],
      ourValues: "Our Values",
      ourValuesSubtitle: "Five principles that guide every partnership and participation we undertake.",
      valuesItems: [
        { title: "Integrity", desc: "We act with honesty and transparency." },
        { title: "Excellence", desc: "We always strive for quality." },
        { title: "Collaboration", desc: "Partnerships create greater impact." },
        { title: "Innovation", desc: "We embrace new solutions." },
        { title: "Impact", desc: "Driven by tangible results." }
      ],
      leadership: "Leadership & Presence",
      leadershipText: "Our headquarters is located in Basra, with offices in Baghdad and Erbil. We work closely with ministries, universities, and private sector leaders.",
      nationwideReach: "Nationwide Reach",
      reachItems: [
        { count: "3 Strategic Offices", label: "Basra • Baghdad • Erbil" },
        { count: "20+ Partner Institutions", label: "Government, Academic & Private" },
        { count: "50+ International Partners", label: "Organizations & Embassies" },
        { count: "100+ Supported Delegations", label: "Representing Iraq Globally" }
      ],
      ceoSectionLabel: "A Word from Leadership",
      ceoName: "Mr. Ahmed Jasim",
      ceoTitle: "Chief Executive Officer, JAZ",
      ceoQuote: "\"We built JAZ on a firm conviction: Iraq deserves genuine international presence. Every exhibition we enter, every partnership we forge, every delegation we support — is a step toward a future where Iraqi professionals are true partners in shaping the global economy.\"",
      ceoTagline: "10+ years of turning ambition into lasting impact",
      ceoStat1Val: "10+",
      ceoStat1Label: "Years of Experience",
      ceoStat2Val: "50+",
      ceoStat2Label: "International Partners",
      ceoStat3Val: "250+",
      ceoStat3Label: "Initiatives Overseen",
      ctaTitle: "Let's build stronger relationships for Iraq.",
      ctaDesc: "Partner with JAZ to access global platforms and create lasting impact.",
      ctaCooperation: "Request Cooperation",
      ctaContact: "Contact Us",
      statPartnerCountriesVal: "15",
      statPartnerCountriesLabel: "Partner Countries",
      statActivePartnersVal: "120",
      statActivePartnersLabel: "Active Partners",
      statJointInitiativesVal: "250",
      statJointInitiativesLabel: "Joint Initiatives",
      statYouthBeneficiariesVal: "1000",
      statYouthBeneficiariesLabel: "Youth Beneficiaries"
    },
    servicesPage: {
      hero: {
        title: "Our Services",
        description: "JAZ coordinates and facilitates bilateral trade, medical delegations, and strategic international partnerships to connect Iraq with global opportunities.",
        ctaCooperation: "Request Cooperation",
        ctaParticipation: "Explore Participation",
        ctaInvitation: "Request Invitation Support"
      },
      stats: {
        expVal: "+10",
        expLabel: "Years of Coordination",
        citiesVal: "Basra • Baghdad • Erbil",
        citiesLabel: "Iraq Offices",
        sectorsVal: "Multi-Sector",
        sectorsLabel: "Active Industries",
        govVal: "Institutional",
        govLabel: "Consular Partnerships"
      },
      coreServices: {
        title: "Service Pillars",
        subtitle: "Four service tracks covering event access, institutional alliances, in-country representation, and consular logistics.",
        items: [
          {
            title: "International Exhibition Participation",
            desc: "We facilitate end-to-end participation in leading global trade exhibitions, including full delegation management — registrations, visa credentials, and consular coordination."
          },
          {
            title: "B2B & Institutional Matchmaking",
            desc: "We establish formal partnerships, joint ventures, and technology transfer agreements between global enterprises, ministries, and regional developers."
          },
          {
            title: "In-Country Market Representation",
            desc: "We provide localized representation and market entry support, connecting international organizations with key government ministries and commerce leaders in Iraq."
          },
          {
            title: "Consular Logistics & Media Integration",
            desc: "We manage on-the-ground VIP protocol, travel logistics, and high-visibility media coverage across JAZ platforms to amplify delegation impact."
          }
        ]
      },
      info: {
        howWeWork: {
          title: "How We Work",
          desc: "Through tailored strategies for international representation."
        },
        industries: {
          title: "Industries We Support",
          desc: "Healthcare, Technology, Commercial and Academic sectors."
        },
        whyJaz: {
          title: "Why JAZ?",
          items: [
            "Trusted global and local networks.",
            "Proven track record of successful outcomes."
          ]
        }
      },
      cta: {
        title: "Let's Build Global Opportunities Together",
        description: "Whether representing an institution or enterprise, JAZ is your partner for global engagement and verified impact.",
        cooperation: "Request Cooperation",
        contact: "Contact Us"
      }
    },
    // Auth
    auth: {
      login: "Login",
      register: "Create New Account",
      logout: "Logout",
      email: "Email",
      password: "Password",
      confirmPassword: "Confirm Password",
      fullName: "Full Name",
      phone: "Phone Number",
      loginSubtitle: "Enter your credentials to access your account",
      registerSubtitle: "Register now to participate in events",
      noAccount: "Don't have an account?",
      hasAccount: "Already have an account?",
      createAccount: "Create Account",
      invalidCredentials: "Invalid email or password",
      passwordMismatch: "Passwords do not match",
      passwordTooShort: "Password must be at least 6 characters",
      verifyEmail: "Verify Your Email",
      verifyEmailMessage:
        "We have sent a verification link to your email. Please click the link to activate your account.",
      backToLogin: "Back to Login",
    },
    // Registration
    registration: {
      title: "Confirm Registration",
      success: "Registration Successful!",
      congratulations:
        "Congratulations! You have been registered for the event",
      ticketNumber: "Your ticket number:",
      keepTicket: "Keep your ticket number to present at entry",
      browseOther: "Browse Other Events",
      registerAs: "You will be registered as:",
      confirm: "Confirm Registration",
      backToEvent: "Back to Event",
      error: "An error occurred during registration. Please try again.",
      loading: "Loading...",
    },
    // Footer
    footer: {
      description:
        "An integrated platform for organizing events, exhibitions, and training and development programs in Iraq.",
      company: "Company",
      team: "Our Team",
      partners: "Partners",
      careers: "Careers",
      services: "Our Services",
      exhibitions: "Exhibitions",
      consulting: "Consulting",
      support: "Support",
      faq: "FAQ",
      privacy: "Privacy Policy",
      terms: "Terms and Conditions",
      rights: "All rights reserved.",
      location: "Baghdad, Iraq",
    },
    // Dashboard
    dashboard: {
      users: "Users",
      registrations: "Registrations",
      settings: "Settings",
      sectorRegistrations: "Sector Registrations",
      messages: "Messages",
    },
    // Common
    common: {
      loading: "Loading...",
      error: "An error occurred",
      save: "Save",
      cancel: "Cancel",
      delete: "Delete",
      edit: "Edit",
      add: "Add",
      search: "Search",
      day: "day",
      hour: "hour",
      minute: "minute",
      home: "Home",
    },
    // Training Page
    trainingPage: {
      badge: "Training and Development Center",
      title: "Training and Development",
      subtitle: "Specialized training programs coming soon",
      features: {
        experts: {
          title: "Globally Certified Experts",
          description: "Real-world expertise and practical skill transfer",
        },
        certificates: {
          title: "Accredited Credentials",
          description: "Recognized certificates locally and internationally",
        },
        interactive: {
          title: "Practical & Applied Methodology",
          description: "Focus on real case studies and immediate application",
        },
        flexible: {
          title: "Flexible Scheduling",
          description: "Tailored timelines that accommodate executive demands",
        },
      },
      programsTitle: "Available Training Programs",
      instructor: "Instructor",
      free: "Free",
      currency: "IQD",
      defaultDescription: "Specialized training program",
      emptyTitle: "Executive Leadership & Capacity Development",
      emptyDescription: "We are currently designing and preparing a comprehensive suite of advanced, high-performance training programs for 2026, in collaboration with elite local and international experts. Pre-register today to secure your seat and be the first to receive program catalogs upon release.",
      emptyButton: "Contact a Training Advisor",
      ctaTitle: "Need a Customized Training Program?",
      ctaDescription: "Programs built around your needs",
      ctaButton: "Contact Us",
    },
    // Time units
    time: {
      day: "day",
      hour: "hour",
      minute: "minute",
    },
  },
} as const;

export type Locale = keyof typeof translations;
export type TranslationKeys = typeof translations.ar;
